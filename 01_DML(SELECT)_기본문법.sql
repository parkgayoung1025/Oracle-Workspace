...
/*
    <LIKE '특정패턴'>
    비교하고자 하는 칼럼 값이 내가 지정한 특정 패턴에 만족 될 경우 조회
    ...
    '%' : 0글자 이상
          비교 대상 칼럼명 LIKE '문자%' => 칼럼 값 중에 '문자'로 시작하는 것을 조회
          비교 대상 칼럼명 LIKE '%문자' => 칼럼 값 중에 '문자'로 끝나는 것을 조회
          비교 대상 칼럼명 LIKE '%문자%' => 칼럼 값 중에 '문자'가 포함되는 것을 조회
          
    '_' : 1글자 이상
          비교 대상 칼럼명 LIKE '_문자' => 칼럼 값 중에 '문자'앞에 무조건 1글자가 존재하는 겨우 조회
          비교 대상 칼럼명 LIKE '__문자' => 칼럼 값 중에 '문자'앞에 무조건 2글자가 존재하는 경우 조회
*/
...
/*
    <IS NULL>
    해당 값이 NULL 인지 비교해 주는 구문
    
    [표현법]
    비교 대상 칼럼 IS NULL : 칼럼 값이 NULL인 경우
    비교 대상 칼럼 IS NOT NULL : 칼럼 값이 NULL이 아닌 경우
*/
...
/*
    <연결 연산자 ||>
    여러 칼럼 값들을 마치 하나의 칼럼인 것처럼 연결시켜주는 연산자
    칼럼과 리터럴(임의의 문자열 값 등)을 연결할 수 있음
...
SELECT *
FROM EMPLOYEE
--ORDER BY BONUS; -- ASC(오름차순) && NULLS LAST 두 개가 기본값
...
SELECT E.*, '하하' AS A
FROM EMPLOYEE E;
-- 모든 칼럼 불러온 것에 추가로 칼럼 더 붙여서 조회 가능
